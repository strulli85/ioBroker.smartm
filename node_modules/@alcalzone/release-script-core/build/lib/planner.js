...
import { DefaultStages } from "./stage.js";
/** Resolve all plugins that are required by the chosen plugins */
export function resolvePlugins(allPlugins, chosenPluginIds) {
...
export async function planStages(context) {
    // Pass 1: collect all graph nodes
    const graphNodes = new Map();
...
export async function planStage(context, stage) {
    ...
    for (const node of graphNodes.values()) {
        ...
        if (stageAfter) {
            ...
            for (const dep of stageAfter) {
                if (!graphNodes.has(dep)) {
                    throw new ReleaseError(`Plugin ${plugin.id} has unknown dependency ${dep} in stage ${stage.id}!`, true);
                }
                node.edges.add(graphNodes.get(dep));
            }
        }
