...
class PackagePlugin {
    id = "package";
    stages = [DefaultStages.check, DefaultStages.edit, DefaultStages.commit];
    ...
    async executeCheckStage(context) {
        ...
        const isMonorepo = "workspaces" in pack && isArray(pack.workspaces) && pack.workspaces.length > 0;
        const lerna = context.hasData("lerna") && !!context.getData("lerna");
        if (isMonorepo) {
            if (lerna) {
                // ok, continue
...
Alternatively, you can use ${context.cli.colors.blue("lerna")} to manage the monorepo.`);
    ...
    async executeEditStageYarnMonorepo(context) {
        ...
        const packageJsonFiles = updatePackages.map((info) => path.join(context.cwd, info.location, "package.json"));
        ...
        if (context.argv.dryRun) {
            ...
            const yarnGte4 = context.hasData("yarn_version") &&
                semver.gte(context.getData("yarn_version"), "4.0.0");
            const commands = [
                publishAll
                    ? [
                        "yarn",
