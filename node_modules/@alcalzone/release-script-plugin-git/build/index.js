...
async function getCurrentBranch(context) {
...
async function getDefaultBranch(context) {
    try {
        const { stdout } = await context.sys.execRaw("git symbolic-ref --short refs/remotes/origin/HEAD", { cwd: context.cwd });
...
async function checkBranchPattern(context, currentBranch) {
    const patterns = context.argv.branchPattern;
    let branchPatterns;
    if (patterns) {
...
class GitPlugin {
    ...
    defineCLIOptions(yargs) {
        return yargs.options({
            ...
            branchPattern: {
                type: "string",
                array: true,
                description: "On which git branches a release may be created. Supports '*' as a wildcard. Defaults to the repository's default branch, or ['main', 'master'] if it cannot be determined.",
            },
        });
    }
    async executeCheckStage(context) {
        ...
        if (!(await hasGitIdentity(context))) {
            const message = `No git identity is configured for the current user ${colors.bold(colors.blue(os.userInfo().username))}!
...
Note: If the current folder belongs to a different user than ${colors.bold(colors.blue(os.userInfo().username))}, you might have to switch to that user first before changing the global config.
`;
            context.cli.fatal(message);
        }
